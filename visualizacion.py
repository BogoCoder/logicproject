...
def dibujar_tablero(f, n):
    ...
    for l in f: # Evaluando cada literal del conjunto
        if '~' not in l: # Tomando los literales verdaderos (los cuales agregan un numero)
            if len(l) <= 2: # Tomando los literales con 'i' perteneciente a {1,...,9}
                if l[0] == 'p':
                ...
                if l[0] == 's':
                    ...
                    axes.add_artist(ab)
            else: # Tomando los literales con 'i' perteneciente a {10,...,16} 
                if l[0] == 'p':
                    ab = AnnotationBbox(imgone, direcciones[int(l[1] + l[2])], frameon=False)
                    axes.add_artist(ab)
                if l[0] == 'q':
                    ab = AnnotationBbox(imgtwo, direcciones[int(l[1] + l[2])], frameon=False)
                    axes.add_artist(ab)
                if l[0] == 'r':
                    ab = AnnotationBbox(imgthree, direcciones[int(l[1] + l[2])], frameon=False)
                    axes.add_artist(ab)
                if l[0] == 's':
                    ab = AnnotationBbox(imgfour, direcciones[int(l[1] + l[2])], frameon=False)
                    axes.add_artist(ab)

    # plt.show()
    fig.savefig("sudoku_" + str(n) + ".png") # Guardando sudokus
...
with open(data_archivo) as csv_file:
    ...
    for l in data:
        print "\n"
        print "Dibujando sudoku 4x4:", l # Dibujando sudoku
